...
type UserLoader struct {
	...
	cache map[string]*gqlmodel.User
...
func (l *UserLoader) Load(key string) (*gqlmodel.User, error) {
...
func (l *UserLoader) LoadThunk(key string) func() (*gqlmodel.User, error) {
	l.mu.Lock()
...
func (l *UserLoader) LoadAll(keys []string) ([]*gqlmodel.User, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserLoader) LoadAllThunk(keys []string) func() ([]*gqlmodel.User, []error) {
	results := make([]func() (*gqlmodel.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserLoader) Prime(key string, value *gqlmodel.User) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserLoader) unsafeSet(key string, value *gqlmodel.User) {
	if l.cache == nil {
		l.cache = map[string]*gqlmodel.User{}
	}
	l.cache[key] = value
}
